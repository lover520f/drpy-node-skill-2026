...
|---|---|---|---|
| L1 | `drpy_check_syntax` + `validate_spider` | 源非语法残档、rule 结构基本合法 | 源可用、建议上传 |
| L2 | `test_spider_interface` 单接口 | 某个接口真实通/断、定位 failure point | 全链路稳定 |
| L3 | `evaluate_spider_source` | 首页→一级→二级→播放→搜索串联评分 | 站点长期稳定 |

结论必须带等级：例如“L2 证据显示 detail 通、play 断”，不要说成“整个源不通”。
...
| `play-debug` | source_name、真实 ids、play_url、flag、detail 输出摘要 | lazy 根因、修改字段、play 复测结果 |
| `repo-upload` | 本地文件路径、A/B/C 建议、L1/L2/L3 证据、标签要求 | file_id、cid、tags、info 核验 |
...
2. 命中播放链 → 调用/执行 `drpy-node-play-debug`，输入 source_name、真实 ids、play_url、flag 和 detail 摘要；回收 lazy 根因与同一 play_url 复测结果。
3. 命中上传/标签 → 调用/执行 `drpy-node-repo-upload`，输入本地文件、A/B/C 建议、L1/L2/L3 证据和标签要求；回收 file_id/cid/tags/info 核验。
